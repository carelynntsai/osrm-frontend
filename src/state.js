...
var State = L.Class.extend({
  ...
  initialize: function(map, lrm_control, lrm_control_custom, tools, default_options, safety) {
    this._lrm = lrm_control;
    // this._lrm_custom = lrm_control_custom;
    this._map = map;
    this._tools = tools;
    this._safety = safety;
    ...
    // this._lrm_custom.getPlan().on('waypointschanged', function() { this.options.waypoints = this._lrm.getWaypoints(); this.update(); }.bind(this));
